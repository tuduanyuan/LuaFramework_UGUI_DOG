...
namespace LuaFramework {
    ...
    public class ResourceManager : Manager {
        ...
        string GetRealAssetPath(string abName) {
            ...
            logger.error("GetRealAssetPath Error:>>" + abName);
            return null;
        }
        ...
        void LoadAsset<T>(string abName, string[] assetNames, Action<UObject[]> action = null, LuaFunction func = null) where T : UObject {
            ...
            //Debugger.Log("载入素材 abName:"+ abName);
            //logger.debug("载入素材 abName:" + abName);
            //logger.debug("载入素材 abName:" + abName);
        ...
        IEnumerator OnLoadAsset<T>(string abName) where T : UObject {
            ...
            if (bundleInfo == null) {
                ...
                if (bundleInfo == null) {
                    ...
                    logger.error("OnLoadAsset--->>>" + abName);
                    yield break;
                }
            }
            //list里面装的就是请求的内容
        ...
        public void UnloadAssetBundle(string abName, bool isThorough = false) {
            abName = GetRealAssetPath(abName);
            logger.debug(m_LoadedAssetBundles.Count + " assetbundle(s) in memory before unloading " + abName);
            UnloadAssetBundleInternal(abName, isThorough);
            UnloadDependencies(abName, isThorough);
            logger.debug(m_LoadedAssetBundles.Count + " assetbundle(s) in memory after unloading " + abName);
        }
        ...
        void UnloadAssetBundleInternal(string abName, bool isThorough) {
            ...
            if (--bundle.m_ReferencedCount <= 0) {
                ...
                logger.debug(abName + " has been unloaded successfully");
            }
        }
    }
}
...
namespace LuaFramework {
    public class ResourceManager : Manager {
        ...
        void LoadDependencies(string name) {
            if (manifest == null) {
                logger.error("Please initialize AssetBundleManifest by calling AssetBundleManager.Initialize()");
                return;
            }
        ...
        void OnDestroy() {
            ...
            logger.debug("~ResourceManager was destroy!");
        }
    }
}
#endif
