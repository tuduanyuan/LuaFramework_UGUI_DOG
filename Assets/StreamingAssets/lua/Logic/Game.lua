...
require "Logic/PanelsSets"
require "Common/functions"
require "Common/trycatchfinally"
require "Controller/PromptCtrl"

local Lanuch = require "Logic/Lanuch"
--管理器--
Game = {};
local this = Game;
...
function Game.OnInitOK()
    logInfo('LuaFramework Game.OnInitOK()--->>>游戏逻辑开始');
    Lanuch:Init();
...
function Game.Run()
    ...
    -- if false and ctrl ~= nil and AppConst.ExampleMode == 1then
    --     ctrl:Awake();
    -- end
    ctrl = CtrlManager.GetCtrl(CtrlNames.Hello);
    if ctrl ~= nil then
        ctrl:Awake();
    end
end
...
function Game.OnLevelWasLoaded(level)
    if level == 1 then
        --主界面场景
        ctrl = CtrlManager.GetCtrl(CtrlNames.Main);
    ...
    if level == 2 then
        ctrl = CtrlManager.GetCtrl(CtrlNames.GameCtrl);
        if ctrl ~= nil then
            log("显示界面:游戏界面")
            ctrl:Awake();
        end
